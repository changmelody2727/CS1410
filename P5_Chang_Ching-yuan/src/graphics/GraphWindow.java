...
public class GraphWindow extends JFrame
{
    ...
    private void loadData ()
    {
        ...
        if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION)
        {
            ...
            {
                // Get new values for column1 and column2
                names = new ArrayList<>();
                values = new ArrayList<>();
                GraphingMethods.readTable(dataSource, names, values);

                // Add the unique categories to the category panel
                categoryPanel.removeAll();
                TreeSet<String> categories = new TreeSet<>(names);
                int colorCount = categories.size();
                int count = 1;
                for (String category : categories)
                {
                    Color color = Color.getHSBColor(1.0f / colorCount * count, 0.75f, 0.75f);
    ...
    private class CategoryArea extends JPanel
    {
        ...
        public ArrayList<Color> getColors ()
        {
            ...
            for (int i = 0; i < checkBoxes.size(); i++)
            {
                ...
                {
                    colors.add(colorButtons.get(i).getBackground());
                }
            }
            return colors;
        }
    ...
    private class DrawingArea extends JPanel
    {
        ...
        public void paintComponent (Graphics graphics)
        {
            ...
            ArrayList<String> categories = categoryPanel.getCategories();
            if (categories.size() > 0)
            {
                ArrayList<Color> colors = categoryPanel.getColors();
                ArrayList<Double> totals = GraphingMethods.summarizeData(names, values, categories,
                        getOperation());
                GraphingMethods.drawGraph(g, categories, totals, colors, pieChart.isSelected());
            }
        }
    }
}
